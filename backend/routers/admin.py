...
async def auto_sync(secret_key: str):
    ...
    asyncio.create_task(combined_background_worker())
...
@router.get("/status")
async def status():
    """Return last sync dates from BigQuery and auto-sync configuration state."""
    auto_sync_configured = bool(AUTO_SYNC_SECRET)
    if not USE_BIGQUERY:
        return {"bigquery": False, "gsc_last_sync": None, "meta_last_sync": None, "auto_sync_configured": auto_sync_configured}
    try:
        ...
        return {"bigquery": True, "gsc_last_sync": gsc_last, "meta_last_sync": meta_last, "auto_sync_configured": auto_sync_configured}
    except Exception as e:
        return {"bigquery": True, "error": str(e), "gsc_last_sync": None, "meta_last_sync": None, "auto_sync_configured": auto_sync_configured}