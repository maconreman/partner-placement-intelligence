...
# Upstash Redis REST API supports a "pipeline"/command-array form that takes
# the key and value out of the URL path entirely and puts them in a JSON body
# instead. This is required for OAuth tokens: the value contains `{`, `"`,
# `:`, and `/` characters, and putting that in a URL path segment causes
# Upstash to 301-redirect to a re-encoded location — httpx follows the
# redirect by default, the second request no longer matches the original
# command, and the write silently never happens. GET-with-value-in-path must
# never be used for _set(); GET is fine for _get()/_del() since no value is
# in play there.
...
async def _set(key: str, value: str, ex: Optional[int] = None) -> None:
    command = ["SET", key, value] + (["EX", str(ex)] if ex else [])
    async with httpx.AsyncClient() as client:
        r = await client.post(_redis_url(), headers=_headers(), json=command)
        r.raise_for_status()
        body = r.json()
        if body.get("error"):
            raise RuntimeError(f"Upstash SET failed for key {key!r}: {body['error']}")


async def _del(key: str) -> None:
    async with httpx.AsyncClient() as client:
        r = await client.post(_redis_url(), headers=_headers(), json=["DEL", key])
        r.raise_for_status()
        body = r.json()
        if body.get("error"):
            raise RuntimeError(f"Upstash DEL failed for key {key!r}: {body['error']}")
