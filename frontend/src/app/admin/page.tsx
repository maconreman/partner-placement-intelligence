...
function StatusPanel({ sync, onRefresh }: { sync: SyncStatus | null; onRefresh: () => void }) {
  const rows = [
    ...
    { label: "Weekly auto-sync", value: sync ? (sync.auto_sync_configured ? "Configured" : "Not configured") : "…", ok: sync?.auto_sync_configured },
  ];
...
function JobPanel({
  ...
  // Elapsed timer — starts when running, resets on each new run.
  useEffect(() => {
    if (!running) return;
    setElapsedSec(0);
    ...
    <div className="panel" style={{ marginBottom: 18 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, marginBottom: 14 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6, flexShrink: 0 }}>
          ...
          {running && (
            <span style={{ fontSize: 12, fontFamily: "var(--mono)", color: "var(--muted)" }}>{elapsedLabel}</span>
          )}
        </div>
      </div>

      <div style={{ fontSize: 12, fontFamily: "var(--mono)", color: "var(--muted)", marginBottom: running || lines.length ? 10 : 0 }}>
        Schedule: {schedule}
      </div>

      {running && (
        <div className="progress-wrap" aria-hidden="true" style={{ marginBottom: 10 }}>
      ...
      {lines.length > 0 && (
        <div
          ref={consoleRef}
          className="console"
          style={{ marginTop: 4, maxHeight: 260 }}
        >
          {lines.map((l, i) => (
            <div
              key={i}
          ...
          {finished === "success" && (
            <div style={{ color: "var(--teal)", marginTop: 6, fontWeight: 600 }}>✓ Job complete — took {elapsedLabel}</div>
          )}
        </div>
      )}
